...
-(NSString *)publicKeyEncryptString:(NSString *)string{
    if(!self.useStringKey){
        if(self.publicKeyPath==nil||self.publicKeyPath.length==0){
            HGBLog(@"公钥文件路径不能为空");
            return nil;
        }
        NSString *path=[HGBRSAEncrytor getCompletePathFromSimplifyFilePath:self.publicKeyPath];
...
-(NSString *)privateKeyDecryptStringString:(NSString *)string{
    if(!self.useStringKey){
        if(self.privateKeyPath==nil||self.privateKeyPath.length==0){
            HGBLog(@"私钥文件路径不能为空");
            return nil;
        }
        if(self.privateKeyPassword==nil||self.privateKeyPassword.length==0){
            HGBLog(@"私钥文件密钥不能为空");
            return nil;
        }
        NSString *path=[HGBRSAEncrytor getCompletePathFromSimplifyFilePath:self.privateKeyPath];
        if(![HGBRSAEncrytor isExitAtFilePath:path]){
            HGBLog(@"私钥文件不存在");
            return nil;
        }
...
-(NSString *)privateKeyEncryptString:(NSString *)string{
    ...
    if(!self.useStringKey){
        ...
        SecKeyRef secKeyRef=[HGBRSAEncrytor getPrivateKeyRefWithContentsOfFile:path password:self.privateKeyPassword];
        if(secKeyRef){
        ...
        NSData *data = [HGBRSAEncrytor encryptData:[string dataUsingEncoding:NSUTF8StringEncoding] withPrivateKey:self.privateKey];
        if(data){
            NSString *ret = base64_encode_data(data);
            return ret;
        }else{
...
-(NSString *)publicKeyDecryptStringString:(NSString *)string{
    ...
    if(!self.useStringKey){
        ...
        NSData *data = [HGBRSAEncrytor decryptData:[string dataUsingEncoding:NSUTF8StringEncoding] withPublicKey:self.publicKey];
        if(data){
            NSString *ret = base64_encode_data(data);
            return ret;
        }else{

...
+(NSMutableData *)ConnectToAPWithSequence:(uint8_t)sequence
{
    ...
    dataByte[1]=0x08;             //不加密,无校验,手机发,没有后续包,ACK
    dataByte[2]=sequence;         //序列
    NSMutableData *data=[NSMutableData dataWithBytes:&dataByte length:sizeof(dataByte)];
    return data;
...
//通知ESP32断开蓝牙连接
+(NSMutableData *)ESP32_Disconnect_BLE:(uint8_t)Sequence
{
    uint8_t dataByte[3];
    dataByte[0]=(0x08<<2) | 0x00; //控制包,通知ESP32断开蓝牙连接
    dataByte[1]=0x00;             //不加密,无校验,手机发,没有后续包,无ACK
    dataByte[2]=Sequence;         //序列
...
//新协商加密数据内容
+(NSData *)SendNewNegotiateData:(NSData *)somedata Sequence:(uint8_t)sequence {
    uint8_t dataByte[4];
    dataByte[0]=(0x00<<2) | 0x01;             //数据包，协商加密方式
    dataByte[1]=0x00 | 0x00;                  //不加密,无校验,手机发,无后续包,无ACK
    dataByte[2]=sequence;                     //序列
    dataByte[3]=somedata.length;                         //data length
...
//SendOpmode更新通知
+(NSData *)SendOpmode:(NSData *)somedata Sequence:(uint8_t)sequence {
    uint8_t dataByte[4];
    dataByte[0]=(0x02<<2) | 0x00;             //数据包，协商加密方式
    dataByte[1]=0x00 | 0x00 |0X00 | 0X00;
    dataByte[2]=sequence;                     //序列
    dataByte[3]=somedata.length;                         //data length
...
//给设备发送协商好的16位密钥
+(NSMutableData *)SendKeyToDevice:(NSData *)randomKeyData Sequence:(uint8_t)sequence Frag:(BOOL)flag TotalLength:(uint16_t)totallength
{
    NSMutableData *data=[[NSMutableData alloc] initWithCapacity:0];
    if (flag) {
        uint8_t dataByte[4];
        dataByte[0]=(0x00<<2) | 0x01;             //数据包，协商加密方式
        dataByte[1]=0x00 | 0x00 | 0x00 | 0x00 | 0x10;                  //不加密,无校验,手机发,无ACK,有后续包,
        dataByte[2]=sequence;                     //序列
        dataByte[3]=randomKeyData.length+2;                         //data length
        ...
        [contentData appendData:randomKeyData];
...
//        [data appendData:checksumdata];
        ...
        dataByte[1]=0x00 | 0x00;                  //不加密,无校验,手机发,无后续包,无ACK
        dataByte[2]=sequence;                     //序列
        dataByte[3]=randomKeyData.length;                         //data length
        
        data=[NSMutableData dataWithBytes:&dataByte length:sizeof(dataByte)];
        //计算校验码
//        NSMutableData *Tempdata=[NSMutableData dataWithData:data];
//        [Tempdata appendData:randomKeyData];
//        NSData *checksumdata=[self GetCRCWithData:Tempdata];
        //拼接
        [data appendData:randomKeyData];
        //加校验码
//        [data appendData:checksumdata];
        return data;
    }
}
//设置softAP 的 username // 现在为了mesh配网改成了发送meshID
+(NSMutableData *)SetMeshID:(NSData *)MeshID Sequence:(uint8_t)sequence Frag:(BOOL)flag Device:(BOOL)device Encrypt:(BOOL)Isencrypt TotalLength:(uint16_t)totallength WithKeyData:(NSData *)keydata
{
    NSMutableData *data=[[NSMutableData alloc] initWithCapacity:0];
    if (flag) {
        uint8_t dataByte[4];
        if (device) {
            dataByte[0]=(0x13<<2) | 0x01;              //新设备数据包，设置meshid
        }else{
            dataByte[0]=(0x09<<2) | 0x01;              //旧设备数据包，设置meshid
        }
        if(Isencrypt)
        {
            dataByte[1]=0x01 | 0x02 |0X08 | 0X10;             //加密,有校验,手机发,有后续包,无ACK
        }else
        {
            dataByte[1]=0x00 | 0x02 | 0X08 | 0X10;             //不加密,有校验,手机发,有后续包,无ACK
        }
        dataByte[2]=sequence;//序列
        dataByte[3]=MeshID.length+2;
        ...
        if(Isencrypt)
        {
            dataByte[1]=0x01 | 0x02 |0X08;             //加密,有校验,手机发,没有后续包,无ACK
        }else
        {
            dataByte[1]=0x00 | 0x02 | 0X08;             //不加密,有校验,手机发,没有后续包,无ACK
        }
        dataByte[2]=sequence;//序列
        dataByte[3]=MeshID.length;//data length
        data=[NSMutableData dataWithBytes:&dataByte length:sizeof(dataByte)];
        //计算校验码
        NSMutableData *Tempdata=[NSMutableData dataWithData:data];
        [Tempdata appendData:MeshID];
        NSData *checksumdata=[self GetCRCWithData:Tempdata];
        
        
        //加密
        if (Isencrypt) {
            Byte *byte=(Byte *)[MeshID bytes];
            MeshID=[DH_AES blufi_aes_Encrypt:sequence data:byte len:(int)MeshID.length KeyData:keydata];
        }
        //拼接
...
+(NSMutableData *)SetStationPassword:(NSString *)password  Sequence:(uint8_t)sequence Encrypt:(BOOL)Isencrypt WithKeyData:(NSData *)keydata
{
    ...
    dataByte[2]=sequence;                    //序列
    dataByte[3]=passworddata.length;             //data length
    NSMutableData *data=[NSMutableData dataWithBytes:&dataByte length:sizeof(dataByte)];
    ...
    if (Isencrypt) {
        ...
        passworddata=[DH_AES blufi_aes_Encrypt:dataByte[2] data:byte len:(int)passworddata.length KeyData:keydata];
    }
    //拼接
...
//设置根节点
+(NSMutableData *)SetRootMesh:(BOOL)root Sequence:(uint8_t)sequence Encrypt:(BOOL)Isencrypt WithKeyData:(NSData *)keydata
{
    uint8_t dataByte[6];
    dataByte[0]=(0x00<<2) | 0x01;       //数据包，设置根节点
    if (Isencrypt) {
        dataByte[1]=0x01 | 0x02;       //加密,有校验,手机发,没有后续包,无ACK
    }else
    {
    ...
    dataByte[2]=sequence;              //序列
    dataByte[3]=0x02;                  //data length
    dataByte[4]=0x05;
    if (root) {
         dataByte[5]=0x01;               //data,根节点
    }else
    {
         dataByte[5]=0x00;               //data,非根节点
    }
    ...
    if (Isencrypt) {
        //加密
        NSData *hh=[DH_AES blufi_aes_Encrypt:dataByte[2] data:&dataByte[4] len:2 KeyData:keydata];
        [data replaceBytesInRange:NSMakeRange(4, 2) withBytes:[hh bytes]];
    }
...
//自定义设置channel值
+(NSMutableData *)SetChannel:(uint8_t)channel Sequence:(uint8_t)sequence Encrypt:(BOOL)Isencrypt WithKeyData:(NSData *)keydata
{
    uint8_t dataByte[6];
    dataByte[0]=(0x00<<2) | 0x01;       //数据包，设置根节点
    if (Isencrypt) {
...
//设置index
+(NSMutableData *)SetIndex:(uint8_t)index Sequence:(uint8_t)sequence Encrypt:(BOOL)Isencrypt WithKeyData:(NSData *)keydata
{
    uint8_t dataByte[6];
    dataByte[0]=(0x00<<2) | 0x01;       //数据包，设置根节点
    if (Isencrypt) {
...
+ (NSMutableData *)SendCustomData:(NSData *)custom_data Sequence:(uint8_t)sequence Frag:(BOOL)flag Encrypt:(BOOL)Isencrypt TotalLength:(uint16_t)totallength WithKeyData:(NSData *)keydata
{
    ...
    if (flag) {
        uint8_t dataByte[4];
        dataByte[0]=(0xa<<2) | 0x01;            //数据包，发送custom data
        if(Isencrypt)
        {
